...
if (!$customerId) {
    jsonError('Customer not found or authentication required', 401);
}

$variantId = !empty($data['variant_id']) ? (int)$data['variant_id'] : null;
$added = addToWishlist((int)$customerId, $data['product_id'], $variantId);

jsonResponse(['success' => $added, 'inWishlist' => true]);
