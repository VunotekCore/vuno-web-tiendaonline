 ...
 *
 * Security:
 *   - Creates backend/.installer-lock after success (blocks re-run)
 *   - Checks if .env already has valid DB credentials
...
if (file_exists(LOCK_FILE)) {
    ...
    if (file_exists($envFile)) {
        ...
        if (!empty($vars['DB_HOST']) && !empty($vars['DB_NAME']) && !empty($vars['DB_USER'])) {
            try {
                $dsn = sprintf('mysql:host=%s;port=%s;dbname=%s;charset=utf8mb4', $vars['DB_HOST'], $vars['DB_PORT'] ?? '3306', $vars['DB_NAME']);
                new PDO($dsn, $vars['DB_USER'], $vars['DB_PASS'] ?? '', [
...
if ($isInstalled) {
    ...
    <body>
        <div class="card">
            ...
            <p>El instalador ya fue ejecutado. Si necesitas reinstalar, eliminá el archivo <code>backend/.installer-lock</code> del servidor y volvé a acceder a esta página.</p>
...
if ($step === 2) {
    ...
    if (!file_exists($schemaFile)) {
        echo renderHeader('Error', 'Instalación', $step);
        echo '<div class="error">No se encontró el archivo <code>backend/database/schema.sql</code>. Verificá que el archivo exista.</div>';
        renderFooter();
        exit;
    }
...
if ($step === 3) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['store_name'])) {
        ...
        if (!$storeName || !$adminEmail || !$adminName || !$adminPass) {
            ...
            try {
                ...
                // Write .env file (replaces database/config.php)
                $envContent = '';
                $envContent .= "# Database — generated by install.php\n";
    ...
    if ($error) echo '<div class="error">' . $error . '</div>';
    ?>
    <p style="font-size:14px;color:#6b6b6b;line-height:1.6;margin-bottom:8px;">Estos datos se guardarán en la base de datos y en <code>.env</code>.</p>
    <form method="post">
        <input type="hidden" name="_step" value="3">
        <input type="hidden" name="db_host" value="<?= htmlspecialchars($dbHost) ?>">
        <input type="hidden" name="db_port" value="<?= htmlspecialchars($dbPort) ?>">
