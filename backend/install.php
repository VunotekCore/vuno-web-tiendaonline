 ...
 *   - Checks if backend/database/config.php already has valid credentials
...
PHP;
    ...
    if ($error) echo '<div class="error">' . $error . '</div>';
    ?>
    <p style="font-size:14px;color:#6b6b6b;line-height:1.6;margin-bottom:8px;">Estos datos se guardarán en la base de datos y en <code>backend/database/config.php</code>.</p>
    <form method="post">
        <input type="hidden" name="_step" value="3">
        <input type="hidden" name="db_host" value="<?= htmlspecialchars($dbHost) ?>">
        <input type="hidden" name="db_port" value="<?= htmlspecialchars($dbPort) ?>">
